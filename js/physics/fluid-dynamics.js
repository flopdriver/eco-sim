...
const FluidDynamicsSystem = {
    ...
    waterFlowRate: 3.5, // Significantly increased for faster water movement
        ...
        for (const pixel of waterPixels) {
            ...
            if (emergencyDrainageNeeded) {
                ...
                if (Math.random() < 0.2) {
                    ...
                    // Only convert to air if above ground, otherwise convert to soil
                    if (pixel.y < groundLevel) {
                        this.physics.core.type[pixel.index] = this.physics.TYPE.AIR;
                        this.physics.core.water[pixel.index] = 0;
                    } else {
                        // Convert to soil if underground
                        this.physics.core.type[pixel.index] = this.physics.TYPE.SOIL;
                        this.physics.core.state[pixel.index] = this.physics.STATE.WET;
                        this.physics.core.water[pixel.index] = Math.min(255, this.physics.core.water[pixel.index]);
                        this.physics.core.nutrient[pixel.index] = 10; // Some basic nutrients
                    }
                    continue;
                }
            }
    ...
    updateSingleWaterPixel: function(x, y, index, nextActivePixels) {
        ...
        // Water above ground evaporates
        else if (y < groundLevel && stuckCounter > 20 && Math.random() < 0.05) {
            this.physics.core.type[index] = this.physics.TYPE.AIR;
            this.physics.core.water[index] = 0;
            return;
        }
        // Water below ground turns to wet soil if stuck for a while
        else if (y > groundLevel && stuckCounter > 30 && Math.random() < 0.05) {
            ...
            switch (this.physics.core.type[downIndex]) {
                ...
                case this.physics.TYPE.SOIL:
                    ...
                    if (this.physics.core.water[downIndex] < soilCapacity) {
                        ...
                        // If water is depleted or nearly depleted, convert to air if above ground, soil if below
                        if (this.physics.core.water[index] <= 2) {
                            if (y < groundLevel) {
                                ...
                                this.physics.core.type[index] = this.physics.TYPE.SOIL;
                                this.physics.core.state[index] = this.physics.STATE.WET;
                                this.physics.core.water[index] = 15; // Minimal water content
                                this.physics.core.nutrient[index] = 10; // Some basic nutrients
                            }
                        } else {
                    ...
                    // Even if soil is saturated, still try to absorb some water
                        ...
                        // If water is depleted, convert to air if above ground, soil if below
                ...
                case this.physics.TYPE.PLANT:
                    ...
                    if (plantState === this.physics.STATE.ROOT) {
                        if (Math.random() < 0.5) {  // Increased absorption chance from 0.3 to 0.5
                            ...
                            // If water is depleted, convert appropriately based on position
                            if (this.physics.core.water[index] <= 0) {
                                if (y < groundLevel) {
                                    // Above ground - convert to air
                                    this.physics.core.type[index] = this.physics.TYPE.AIR;
                                    ...
                                    this.physics.core.type[index] = this.physics.TYPE.SOIL;
                                    this.physics.core.state[index] = this.physics.STATE.WET;
                                    this.physics.core.water[index] = 15;
                    ...
                    // Stems can absorb water or let it pass through
                        ...
                        // If stem didn't absorb water, it might let water pass through (rarely)
                            ...
                            if (belowStemIndex !== -1 &&
                                ...
                                // Convert original water to air or soil based on position
                                if (y < groundLevel) {
                                    // Above ground - convert to air
                                    this.physics.core.type[index] = this.physics.TYPE.AIR;
                                    this.physics.core.water[index] = 0;
        ...
        // Check ground level to enhance spreading on the ground (defined above)
        ...
        if (isNearGround) {
            ...
            // Try first direction
            if (checkLeftFirst) {
                if (this.tryMoveWaterHorizontal(index, leftIndex, nextActivePixels, groundLevel)) return;
                if (this.tryMoveWaterHorizontal(index, rightIndex, nextActivePixels, groundLevel)) return;
            } else {
                if (this.tryMoveWaterHorizontal(index, rightIndex, nextActivePixels, groundLevel)) return;
                if (this.tryMoveWaterHorizontal(index, leftIndex, nextActivePixels, groundLevel)) return;
            }
        ...
        if (checkLeftFirst) {
            if (this.tryMoveWaterDiagonal(index, downLeftIndex, nextActivePixels, groundLevel)) return;
            if (this.tryMoveWaterDiagonal(index, downRightIndex, nextActivePixels, groundLevel)) return;
        } else {
            if (this.tryMoveWaterDiagonal(index, downRightIndex, nextActivePixels, groundLevel)) return;
            if (this.tryMoveWaterDiagonal(index, downLeftIndex, nextActivePixels, groundLevel)) return;
        }
        ...
        if (stuckCounter > 10) {
            ...
            for (const neighbor of neighbors) {
                if (this.physics.core.type[neighbor.index] === this.physics.TYPE.AIR) {
                    ...
                    // If below ground, convert to soil instead of air
                    if (y >= groundLevel) {
                        this.physics.core.type[index] = this.physics.TYPE.SOIL;
                        this.physics.core.state[index] = this.physics.STATE.WET;
                        this.physics.core.water[index] = 15;
            ...
            // If still stuck and underground for 20+ ticks, just convert to soil
    ...
    tryMoveWaterHorizontal: function(fromIndex, toIndex, nextActivePixels, groundLevel) {
        ...
        // Get position info to check if we're below ground
    ...
    tryMoveWaterDiagonal: function(fromIndex, toIndex, nextActivePixels, groundLevel) {
        ...
        if (this.physics.core.type[toIndex] === this.physics.TYPE.AIR) {
            ...
            if (Math.random() < moveProbability) {
                ...
                // Clear source pixel - convert to soil if below ground
                if (isBelowGround) {
                    this.physics.core.type[fromIndex] = this.physics.TYPE.SOIL;
                    this.physics.core.state[fromIndex] = this.physics.STATE.WET;
                    this.physics.core.water[fromIndex] = 15;
